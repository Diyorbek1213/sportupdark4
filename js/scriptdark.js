...
const menuDots = document.querySelectorAll('.menu__item__link__dots');
const menuList = document.querySelectorAll('.menu__item__list');
const menuSublink = document.querySelectorAll('.menu__item__sublink');
if (menuDots && menuList && menuSublink) {
	for (let i = 0; i < menuDots.length; i++) {
		menuDots[i].addEventListener("mouseover", (e) => {
			menuSublink[i].classList.add('_active');
			menuList[i].addEventListener("mouseleave", () => {
				menuSublink[i].classList.remove('_active');
			})
		})
	}
}
